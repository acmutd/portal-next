...
const ApplicationCard: FC<ApplicationCardProps> = ({
  ...
  return (
    <div className="flex flex-col items-end w-fit">
      ...
      <div className="w-80 h-48 p-6 rounded-3xl space-y-2 flex flex-col justify-between bg-gray-200/5 outline outline-gray-100/10 overflow-hidden hover:overflow-auto scrollbar-hide">
        <div>
        ...
        <div className="flex gap-1 ml-auto w-fit">
          {buttons.map((button) => (
            <div key={Math.random()} className="relative w-fit">
              {button}
            </div>
          ))}
        </div>
